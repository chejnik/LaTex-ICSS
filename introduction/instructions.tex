...
\def\tableB{\noindent\begin{tabularx}{\columnwidth}{>{\bfseries}lX}
  ...
  (-a, -ar)           & označuje deklinační\,/\addthin konjugační koncovky hesla \\
...
\def\tableC{\noindent\begin{tabularx}{\columnwidth}
    {>{\bfseries}l>{\footnotesize\itshape}l>{\small}l>{\small}l>{\small}l>{\small}l}
  \toprule
...
K~dělení heslových slov je použita střední tečka \uv{\textbf{·}}, která odděluje části složeného slova, přípony a~předpony. Platí následující pravidla:

\renewcommand{\labelenumi}{\alph{enumi})}
\begin{enumerate}
\item jediné dělení heslového slova je označeno dvěma středními tečkami \uv{\textbf{··}}. Může se jednat o~dělení částí složeného slova
\blspace
...
\blspace
\item V~případě, že se v~hesle vyskytuje více dělení, jsou použity dvě střední tečky \uv{\textbf{··}} pro hlavní dělení a~jedna střední tečka \uv{\textbf{·}} pro zbylé\,/\addthin zbylá dělení.
\blspace
...
\blspace
  \dicEntry \dicTerm{af} \dicPos{prep\,/\addthin adv}
\blspace
...
\blspace
  \dicEntry \dicTerm{klóra} \dicPos{v} \dicFlx{acc\,/\addthin dat}
\blspace
...
\blspace
  \dicEntry \dicTerm{af} \dicPos{prep\,/\addthin adv} \dicFlx{dat}
\blspace

V~případě, že se předložka pojí s~více pády, je použito lomítko.

\blspace
  \dicEntry \dicTerm{fyrir} \dicPos{prep\,/\addthin adv} \dicFlx{acc\,/\addthin dat}
\blspace
...
\blspace
  \dicEntry \dicTerm{beð|ur} \dicPos{m} \dicFlx{(-s\,/\addthin -jar, -ir)}
\blspace
...
kde \uv{\textit{hvals}} je 2.~pád jednotného čísla a~\uv{\textit{hvalir}} 1.~pád množného čísla.
...
Pokud přídavné jméno v~ženském rodě neztrácí koncové \uv{\textit{-r}}, je uveden tvar hesla v~ženském rodě takto:
...
\textls[24]{Pokud má přídavné jméno nepravidelné tvary} 2.~a~3.~stupně stupňování, jsou tyto tvary uvedeny v~závorce spolu se zkratkou pro 2.~stupeň (\textbf{comp}) a~3.~stupeň (\textbf{sup}).
...
Islandská slovesa se dělí na slabá, silná a~nepravidelná. Vzhledem k~tomuto dělení se liší počet slovních tvarů uvedených v~závorce za zkratkou \textbf{v}. Slovní tvary se uvádějí proto, aby si podle nich uživatel mohl
...
Slabá slovesa, která tvoří 1.~osobu jednotného čísla minulého času oznamovacího způsobu koncovkou \uv{\textit{-aði}}, mají v~závorce uvedenu jen jednu koncovku, jmenovitě \uv{\textit{(-aði)}}.
...
Zkratka \textbf{refl} označuje tvar středního slovesného rodu.
...
Čísla podtříd slouží pro rozlišení odlišných skloňování\,/\addthin časování.
...
V~některých případech je v~české části použita závorka. V~prvním případě je závorka užívaná ke~zkrácení zápisu dvou slov (\uv{\textit{pomoc}}, \uv{\textit{výpomoc}}).
...
\blspace
  \dicEntry \dicTerm{aðal··|gata} \dicPos{f} \dicFlx{(-götu, -götur)} \textCS{hlavní třída\,/\addthin ulice}
\blspace

Význam přečteme jako \uv{\textit{hlavní třída, hlavní ulice}}.
...
Příklad můžeme přečíst jako \uv{\textit{taka djúpt í árinni}} nebo \uv{\textit{taka of djúpt í árinni}}.
...
\blspace
\dicEntry \dicTerm{finna} \dicPos{v} \dicFlx{(finn, fann, fundum, fyndi, fundið)} \\
\dicIdiomIntro{finnast} {\dicPhraseIS{finnast}} \dicFlx{refl} {být nalezen, najít se, nalézt se, nacházet se}
\blspace
...
\blspace
\dicEntry \dicTerm{koma\textsuperscript{2}} \dicPos{v} \dicFlx{(kem, kom, komum, kæmi, komið)} \\
\dicIdiomIntro{koma}[að] {\dicPhraseIS{koma að e-m}} {přiblížit se ke (komu), přijít ke (komu)}
\blspace
...
\blspace
\dicEntry \dicTerm{kast} \dicPos{n} \dicFlx{(-s, köst)} \\
\dicIdiomIntro{kast} {\dicPhraseIS{þegar til kastanna kemur}} {\footnotesize{přen.}} {když jde do tuhého}
\blspace
...
\blspace
\dicEntry \dicTerm{æfing} \dicPos{f} \dicFlx{(-ar, -ar)} \\
\dicProverbIntro{} \dicPhraseIS{Æfing skapar meistara.} {\footnotesize{přís.}} \textCS{Cvičení dělá mistra.}
\end{enumerate}
...
\blspace
  \dicEntry \dicTerm{drag} {\dicPos{n} \dicFlx{(-s, drög)} {\textbf{1.}} {mokřina, podmáčený terén} {\textbf{2.}} \textbf{drög} {\footnotesize{pl}} {\textbf{a.}} \dicSynonym{uppsprettur} {prameny (řeky ap.)} {\textbf{b.}} \dicSynonym{undirbúningur} {náčrt, návrh.}}
\blspace
...
\blspace
  \dicEntry \dicTerm{tím|i} \dicPos{m} \dicFlx{(-a, -ar)} {\textbf{1.}} \dicSynonym{tíð} {čas, doba}; \textbf{í þann tíma} \dicFlx{adv} \dicSynonym{þá\textsuperscript{2}} {v~té době, tou dobou} {\textbf{2.}} \dicSynonym{rétt stund} {(správný) čas\,/\addthin moment}
\blspace
...
Nakonec jsou uvedena přísloví a~rčení označená symbolem {\color{newdev}\ding{96}}.

\blspace
  \dicEntry \dicTerm{eik} \dicPos{n} \dicFlx{(-ar\,/\addthin -ur, -ur)} \dicProverb{} \dicPhraseIS{Ekki\,/\addthin eigi fellur eik við fyrsta högg.} {\footnotesize{přís.}} {Jednou ranou dub nespadne.}
\blspace
...
\blspace
  \dicEntry \dicTerm{staða} \dicPos{f} \dicFlx{(stöðu, stöður)} {\textbf{1.}} \dicSynonym{ástand} {situace, stav, poměry} {\textbf{2.}} \dicSynonym{embætti} {post, (pracovní) místo\,/\addthin pozice (ve firmě ap.)}
\blspace
...
\blspace
  \dicEntry \dicTerm{leit|a} \dicPos{v} \dicFlx{(-aði)} \textbf{leita að e-u\,/\addthin e-m} {hledat (co\,/\addthin koho)} \dicExampleIS{leita að lyklunum} \dicExampleCS{hledat klíče}
\blspace

Příklad tedy přečteme jako \uv{\textit{leita að einhverju}} (\textit{hledat co}) a~\uv{\textit{leita að einhverjum}} (\textit{hledat koho}). V~tomto případě se sloveso pojí s~3.~pádem a~předmět může být životný i~neživotný.
...
\blspace
\dicEntry \dicTerm{tím|i} \dicPos{m} \dicFlx{(-a, -ar)} \textbf{3.} \dicSynonym{klukkustund} {hodina} {\footnotesize{(šedesát minut)}} \textls[-10]{\dicExampleIS{tveggja tíma gangur} \dicExampleCS{dvouhodinový pochod}}
\blspace

\item Věta jako praktický příklad:

\blspace
\dicEntry \dicTerm{mæt|a} \dicPos{v} \dicFlx{(-ti, -t)} \\
\dicIdiomIntro{mætast} \dicPhraseIS{mætast} \dicSynonym*{ná saman} \dicFlx{refl} {setkat se, potkat se} \dicExampleIS{Þau mættust á gangstéttinni.} \dicExampleCS{Potkali se na chodníku.}
\end{enumerate}
...
\fbox{\includegraphics[width=5cm]{ds_image_posthus_0_2.jpg}}\end{center}
